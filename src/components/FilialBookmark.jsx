...
const FilialBookmark = ({ filialId, updatedBookmarks, filialText}) => {
    ...
    const sendArchiveRequest = async () => {
        setLoading(true);
        try {
            const response = await fetch(`https://cargotrek.kz:3001/api/archive/${userId}/archive`, {
                method: 'POST',
                headers: {
    ...
    const removeBookmark = async (trackNumber) => {
        ...
        if (confirmDelete) {
            try {
                const response = await fetch(`https://cargotrek.kz:3001/api/bookmark/${userId}/delete/${trackNumber}`, {
                    method: 'DELETE',
                });
    ...
    return (
        <div className="status-detail">
             <div key={filialId} className="status-client" onClick={() => setShowDetail(!showDetail)}>    
                <p className="status__title">{filialText}</p>
                <div className="quantity"><p className="quantity__p">{updatedBookmarks.filter(bookmark => bookmark.currentStatus === filialId).length}</p></div>
            </div>
            ...
            {showDetail && (
                                ...
                                <div className="ul-detail-border">
                                    <li key={index} className="li-track-detail" >
                                        ...
                                        <ul className="date-li">
                                            <b className="date-text"> {filialText}:</b>  
                                        {bookmark.history
                                            .filter(historyItem => historyItem.status.toString() === filialId.toString())
                    ...
                    <div className="area"></div>
                            ...
                            {filteredBookmarks.length > 0 && filialText === "Получено" && (
